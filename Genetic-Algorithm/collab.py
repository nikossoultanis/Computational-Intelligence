...
def fitness_function(chromosome, solution_index):  # get the chromosome
    ...
    for i in range(10):  # calculate the pearson helper for each other user
        ...
        prod = abs(np.corrcoef(chromosome, neighbor)[0, 1])  # and now calculate the corrcoef
        r.append(prod)
    fit = np.mean(r)
...
def perform_crossover(m, f):  # m is mother and f is father
    cross_prob = 90 # probability of crossover
    roll = random.randrange(1, 100)
    if roll <= cross_prob:
...
population = initialize_population(200) # initialize population of n-chromosomes
